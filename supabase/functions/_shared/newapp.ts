...
const SYSTEM_COLUMNS = "id,name,brand,inverter_brand,capacity_kw,api_brand,api_station_id,api_device_sn,api_credentials,has_consumption_meter,has_battery";
...
function findValue(source: unknown, aliases: string[]): { key: string; value: unknown } | null {
  ...
  const visit = (value: unknown): { key: string; value: unknown } | null => {
    ...
    for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
      if (wanted.has(normalizeKey(key)) && nested !== null && nested !== undefined && nested !== "") return { key, value: nested };
    }
...
function normalizeEnergy(reading: Reading, system: SystemConfig): Reading {
  ...
  output.soc_pct = output.soc_pct === null || output.soc_pct === undefined ? null : Math.max(0, Math.min(100, toNumber(output.soc_pct) ?? 0));
  ...
  const hasBattery = Boolean(system.has_battery || output.soc_pct !== null || output.battery_power_w !== null);
  ...
  } else if (output.consumption_w !== null && output.consumption_w !== undefined && output.battery_power_w !== null && output.battery_power_w !== undefined && gridNet === null) {
  ...
  } else if (!hasBattery && output.consumption_w !== null && output.consumption_w !== undefined && gridNet === null) {
    const inferred = output.consumption_w - solar;
    output.grid_import_w = inferred >= 0 ? Math.round(inferred) : 0;
    output.grid_export_w = inferred < 0 ? Math.round(Math.abs(inferred)) : 0;
  }
  output._raw = {
    ...(output._raw && typeof output._raw === "object" && !Array.isArray(output._raw) ? output._raw as Record<string, unknown> : { value: output._raw ?? null }),
    _edge_runtime: true,
  };
  return output;
}

async function logUsage(system: SystemConfig, status: number, durationMs: number, error?: unknown, requestsCount = 1): Promise<void> {
...
async function recordFailure(system: SystemConfig, error: unknown, startedAt: number, requestsCount = 1): Promise<void> {
...
async function persistReading(system: SystemConfig, reading: Reading, startedAt: number, requestsCount = 1): Promise<void> {
  ...
  if (normalized.consumption_w !== null && normalized.consumption_w !== undefined && !system.has_consumption_meter) updates.has_consumption_meter = true;
  if ((normalized.soc_pct !== null && normalized.soc_pct !== undefined) || (normalized.battery_power_w !== null && normalized.battery_power_w !== undefined)) {
    if (!system.has_battery) updates.has_battery = true;
  }
  if (Object.keys(updates).length) {
    await db().from("newapp_systems").update({ ...updates, updated_at: new Date().toISOString() }).eq("id", system.id);
  }
...
async function deyePost(path: string, body: unknown, token?: string): Promise<any> {
  const [baseUrl, appId] = await Promise.all([
    newappSecret("DEYE_API_URL"), newappSecret("DEYE_APP_ID"),
  ]);
  if (!baseUrl || !appId) throw new Error("DEYE API URL/app id missing");
  const { data } = await fetchJson<any>(`${baseUrl.replace(/\/$/, "")}${path}?appId=${encodeURIComponent(appId)}`, {
    method: "POST",
...
async function deyeToken(): Promise<string> {
  const [appSecret, email, password] = await Promise.all([
    newappSecret("DEYE_APP_SECRET"), newappSecret("DEYE_EMAIL"), newappSecret("DEYE_PASSWORD"),
  ]);
  ...
  const organization = organizations.find((item: any) => String(item.companyName ?? "").toLowerCase().includes("solar")) ?? organizations[0];
...
function deyeValue(list: any[], ...keys: string[]): number | null {
  for (const key of keys) {
    const found = list.find((item) => typeof item?.key === "string" && item.key.toLowerCase() === key.toLowerCase());
    const value = toNumber(found?.value);
    if (value !== null) return value;
  }
  return null;
...
function mapDeye(dataList: any[], stationId: string, sn: string): Reading {
  ...
  const batteryItem = dataList.find((item) => ["batterypower", "batterychargepower"].includes(String(item?.key ?? "").toLowerCase()));
  const batteryValue = toNumber(batteryItem?.value);
  const batteryPower = batteryValue === null ? null : (batteryItem.key === "batteryPower" ? -batteryValue : batteryValue);
  return {
    power_w: deyeValue(dataList, "TotalSolarPower", "TotalActiveACOutputPower", "SolarPower", "totalDcInputPower"),
    energy_kwh: deyeValue(dataList, "DailyActiveProduction", "DailyEnergy", "TodayEnergy", "dailyProductionActive"),
    ...
    temperature_c: deyeValue(dataList, "AC Temperature", "DeviceTemperature", "Device_Temperature", "Temperature_Box", "acTemperature", "Temperature- Battery", "BatteryTemperature", "temperatureBattery"),
    frequency_hz: deyeValue(dataList, "ACOutputFrequencyR", "GridFrequency", "AC_Frequency", "Grid_Frequency"),
...
async function pollDeye(systems: SystemConfig[]): Promise<Map<string, Reading | Error>> {
  ...
  try {
    ...
    for (const system of systems) if (system.api_device_sn) snByStation.set(String(system.api_station_id), system.api_device_sn);
    for (let index = 0; index < stationIds.length; index += 10) {
      ...
      try {
        ...
        for (const stationId of chunk) {
          ...
          const inverter = matching.find((device: any) => device.deviceSn && String(device.deviceType ?? "").toUpperCase() === "INVERTER") ?? matching.find((device: any) => device.deviceSn);
    ...
    for (let index = 0; index < sns.length; index += 10) {
      ...
      for (const device of data?.deviceDataList ?? []) dataBySn.set(String(device.deviceSn), device.dataList ?? []);
    }
    let stations: any[] = [];
      ...
      if (sn && dataList?.length) {
        ...
        if (station) {
          results.set(system.id, {
            power_w: toNumber(station.generationPower), soc_pct: toNumber(station.batterySOC),
    ...
    for (const system of systems) results.set(system.id, error instanceof Error ? error : new Error(String(error)));
  }
  return results;
}
...
async function enphaseToken(): Promise<string> {
  const [clientId, clientSecret, email, password] = await Promise.all([
    newappSecret("ENPHASE_CLIENT_ID"), newappSecret("ENPHASE_CLIENT_SECRET"),
    newappSecret("ENPHASE_EMAIL"), newappSecret("ENPHASE_PASSWORD"),
  ]);
...
async function enphaseGet(path: string, token: string, params: Record<string, string | number> = {}): Promise<any> {
  const [baseUrl, apiKey] = await Promise.all([newappSecret("ENPHASE_API_URL"), newappSecret("ENPHASE_API_KEY")]);
  if (!baseUrl || !apiKey) throw new Error("ENPHASE API URL/key missing");
  const query = new URLSearchParams({ key: apiKey, ...Object.fromEntries(Object.entries(params).map(([key, value]) => [key, String(value)])) });
...
async function fetchEnphase(system: SystemConfig, token: string, closing: boolean): Promise<{ reading: Reading; requests: number }> {
  ...
  if (closing && system.has_consumption_meter) {
    ...
    const safe = async (endpoint: string) => {
      try {
        requests += 1;
        return await enphaseGet(`/systems/${systemId}/telemetry/${endpoint}`, token, { start_at: start, end_at: end });
      } catch (error) {
    ...
    [consumption, imported, exported] = await Promise.all([
      safe("consumption_meter"), safe("energy_import_telemetry"), safe("energy_export_telemetry"),
    ]);
  }
  ...
  return {
    ...
    reading: {
      ...
      _raw: {
        summary,
        telemetry: { consumption_meter: consumption, energy_import_telemetry: imported, energy_export_telemetry: exported },
        ...(closing ? { _daily: {
...
async function fusionSession(): Promise<{ baseUrl: string; token: string }> {
  const [baseUrl, userName, systemCode] = await Promise.all([
    newappSecret("FUSION_SOLAR_API_URL"), newappSecret("FUSION_SOLAR_USER"), newappSecret("FUSION_SOLAR_PASSWORD"),
  ]);
  ...
  const response = await fetchJson<any>(`${baseUrl.replace(/\/$/, "")}/login`, {
    method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ userName, systemCode }),
  }, "FusionSolar login");
  if (response.data?.failCode !== 0) throw new Error(`FusionSolar login: ${response.data?.message ?? response.data?.failCode}`);
...
async function fusionPost(session: { baseUrl: string; token: string }, endpoint: string, body: unknown): Promise<any> {
...
async function deriveFusionPower(systemId: string, daily: Record<string, number | null>): Promise<Record<string, number | null>> {
  ...
  return {
    ...
    grid_export_w: watts(daily.export, previous.data.daily_export_kwh ?? previous.data.raw_response?._daily?.day_grid_export_kwh),
    grid_import_w: watts(daily.import, previous.data.daily_import_kwh ?? previous.data.raw_response?._daily?.day_grid_import_kwh),
  };
}

async function fetchFusion(system: SystemConfig, session: { baseUrl: string; token: string }): Promise<Reading> {
  ...
  return {
    ...
    _raw: { _station: kpi, _daily: {
      day_production_kwh: daily.production, day_consumption_kwh: daily.consumption,
      day_grid_export_kwh: daily.export, day_grid_import_kwh: daily.import,
      month_production_kwh: toNumber(kpi.month_power), total_production_kwh: toNumber(kpi.total_power),
...
async function fetchShine(system: SystemConfig): Promise<Reading> {
  ...
  const [secretAccount, secretPassword, configuredUrl] = await Promise.all([
    newappSecret("SEMS_ACCOUNT"), newappSecret("SEMS_PASSWORD"), newappSecret("SHINE_API_URL"),
  ]);
  ...
  const login = await fetchJson<any>("https://hk.semsportal.com/api/v2/Common/CrossLogin", {
    method: "POST", headers: { "content-type": "application/json", Token: JSON.stringify({
      version: "v2.1.0", client: "ios", language: "en", timestamp: String(Math.floor(Date.now() / 1000)), uid: "", token: "",
  ...
  const monitor = await fetchJson<any>(`${baseUrl}/v3/PowerStation/GetMonitorDetailByPowerstationId`, {
    method: "POST", headers: { "content-type": "application/json", Token: JSON.stringify({
      version: "", client: "ios", language: "en", timestamp: String(Math.floor(Date.now() / 1000)),
  ...
  return {
    ...
    consumption_w: pickWatts(sources, ["load_power", "load", "pload", "p_to_load", "home_load_power", "consumption_power"]),
    grid_import_w: pickWatts(sources, ["grid_in_power", "grid_import_power", "import_power", "buy_power", "p_from_grid"]) ?? (signedGrid !== null && signedGrid > 0 ? signedGrid : null),
    grid_export_w: pickWatts(sources, ["grid_out_power", "grid_export_power", "export_power", "sell_power", "p_to_grid"]) ?? (signedGrid !== null && signedGrid < 0 ? Math.abs(signedGrid) : null),
    soc_pct: pickNumber(sources, ["soc", "soc_text", "battery_soc"]),
    battery_power_w: pickWatts(sources, ["b_total_charge_power", "battery_power", "pbat_w", "pbat", "battery_charge_power"]),
    _raw: monitor.data,
  };
}
...
async function growattGet(path: string, params: Record<string, string>, token: string): Promise<any> {
  ...
  if (data?.error_code && data.error_code !== 0) throw new Error(`GROWATT ${path}: ${data.error_msg ?? data.error_code}`);
...
async function fetchGrowatt(system: SystemConfig): Promise<Reading> {
  ...
  const deviceRows = Array.isArray(devices?.devices) ? devices.devices : Array.isArray(devices) ? devices : [];
  const device = deviceRows.find((item: any) => String(item.device_sn ?? item.deviceSn ?? item.sn) === String(system.api_device_sn)) ?? deviceRows[0] ?? {};
  const sources = [plant, device, selected];
  return {
    power_w: toNumber(selected.current_power) ?? (toNumber(plant?.current_power) === null ? null : toNumber(plant.current_power)! * 1000),
    energy_kwh: toNumber(plant?.today_energy) ?? pickNumber(sources, ["today_energy", "daily_energy", "eToday", "eday"]),
...
function isGrowatt(system: SystemConfig): boolean {
  return system.api_brand === "GROWATT" || system.api_credentials?.provider === "growatt" ||
    String(system.brand ?? "").toUpperCase() === "GROWATT" || String(system.inverter_brand ?? "").toUpperCase() === "GROWATT";
}
...
async function throttleNight(systems: SystemConfig[]): Promise<SystemConfig[]> {
  ...
  const latest = await lastReadingBySystem(systems.map((system) => system.id), new Date(Date.now() - 70 * 60_000).toISOString());
...
export async function pollFast(): Promise<TaskResult> {
  ...
  if (fusionSystems.length) {
    try { fusion = await fusionSession(); } catch (error) {
      for (const system of fusionSystems) await recordFailure(system, error, Date.now());
      failed += fusionSystems.length;
    }
  }
  for (const system of selected.filter((item) => item.api_brand !== "DEYE" && item.api_brand !== "FUSION_SOLAR")) {
    const started = Date.now();
    try {
  ...
  return { processed: selected.length, succeeded, failed, nighttime: argentinaHour() >= 19 || argentinaHour() < 7 };
}
...
export async function pollEnphase(): Promise<TaskResult> {
  ...
  if (requestsUsed >= limit) return { processed: 0, succeeded: 0, failed: 0, skipped_limit: due.length, requestsUsed };
  let succeeded = 0;
  let failed = 0;
  let token: string;
  try { token = await enphaseToken(); } catch (error) {
  ...
  for (const system of due) {
    ...
    try {
      ...
      if (requestsUsed + result.requests > limit) throw new Error(`ENPHASE daily request limit ${limit} reached`);
...
export async function consolidate(date: "today" | "yesterday"): Promise<TaskResult> {
  ...
  if (date === "yesterday") {
    await db().from("newapp_rental_payments").update({ status: "overdue", updated_at: new Date().toISOString() })
...
async function disableFailingSystems(): Promise<void> {
  ...
  for (const system of systems) {
    ...
    if (alarm.data) await db().from("newapp_systems").update({ polling_enabled: false, updated_at: new Date().toISOString() }).eq("id", system.id);
  }
}
...
export async function updateUva(): Promise<TaskResult> {
  ...
  for (let offset = 0; offset >= -7; offset -= 1) {
    ...
    try {
      const { data } = await fetchJson<any>(`${baseUrl}/Monetarias/${variable}?desde=${date}&hasta=${date}`, {}, "BCRA UVA");
      ...
      const stored = await db().from("newapp_uva_values").upsert({
        date: row.fecha ?? date, value_ars: value, source: offset === 0 ? "bcra" : "bcra_fallback", fetched_at: new Date().toISOString(),
      }, { onConflict: "date" });
...
export async function sendMorningReport(): Promise<TaskResult> {
  ...
  const [daily, readings, alarms, uva] = await Promise.all([
    ...
    db().from("newapp_energy_readings").select("system_id,read_at").in("system_id", ids)
      .gte("read_at", new Date(Date.now() - 2 * 3_600_000).toISOString()).order("read_at", { ascending: false }).limit(2000),
    db().from("newapp_alarms").select("alarm_type,severity,message").eq("is_active", true).order("created_at", { ascending: false }).limit(20),
    db().from("newapp_uva_values").select("date,value_ars").order("date", { ascending: false }).limit(1).maybeSingle(),
  ]);
  ...
  for (const row of readings.data ?? []) if (!latest.has(row.system_id)) latest.set(row.system_id, row.read_at);
  ...
  for (const system of systems) {
    ...
    lines.push(`${icon} ${system.name}: ${Number(production.get(system.id) ?? 0).toFixed(1)} kWh; ${Number.isFinite(minutes) ? `lectura hace ${minutes} min` : "sin lectura 2h"}`);
  }
  const alarmRows = alarms.data ?? [];
  lines.push("", alarmRows.length ? `🚨 *Alarmas activas: ${alarmRows.length}*` : "✅ Sin alarmas activas");
  for (const alarm of alarmRows.slice(0, 5)) lines.push(`${alarm.severity === "critical" ? "🚨" : "⚠️"} ${alarm.message}`);
  if (uva.data) lines.push("", `UVA: $${Number(uva.data.value_ars).toLocaleString("es-AR", { maximumFractionDigits: 2 })} al ${uva.data.date}`);
  const phones = String(await newappSecret("MORNING_REPORT_PHONES") ?? "").split(",").map((value) => value.replace(/[^0-9]/g, "")).filter(Boolean);
  if (!phones.length) throw new Error("MORNING_REPORT_PHONES is not configured");
  let succeeded = 0;
  let failed = 0;
  for (const phone of phones) {
    try { await sendPulseText("newapp", newappSecret, phone, lines.join("\n")); succeeded += 1; }
    catch (error) { console.error("NEWAPP morning report", error); failed += 1; }
  }
...
async function discoverDeye(): Promise<Discovered[]> {
  ...
  return stations.map((station: any) => ({
    ...
    brand: "DEYE", inverter_brand: "DEYE", api_brand: "DEYE",
    api_station_id: String(station.id ?? station.stationId), api_device_sn: null,
...
async function discoverEnphase(): Promise<Discovered[]> {
  ...
  return systems.map((system: any) => ({
    ...
    brand: "ENPHASE", inverter_brand: "Enphase", api_brand: "ENPHASE",
    api_station_id: String(system.system_id), api_device_sn: null,
...
async function discoverFusion(session: { baseUrl: string; token: string }): Promise<Discovered[]> {
  ...
  return stations.map((station: any) => {
    ...
    return {
      ...
      brand: "FUSION_SOLAR", inverter_brand: "Huawei", api_brand: "FUSION_SOLAR",
      api_station_id: String(station.stationCode), api_device_sn: null, capacity_kw: capacity,
    };
  });
}
...
async function discoverGrowatt(): Promise<Discovered[]> {
  ...
  return plants.map((plant: any) => ({
    ...
    brand: "GROWATT", inverter_brand: "Growatt", api_brand: "SHINE",
    api_station_id: String(plant.plant_id ?? plant.plantId ?? plant.id), api_device_sn: null,
...
export async function discoverSystems(): Promise<TaskResult> {
  const tasks = await Promise.allSettled([
    discoverDeye(), discoverEnphase(), fusionSession().then(discoverFusion), discoverGrowatt(),
  ]);
  const discovered: Discovered[] = [];
  let failed = 0;
  for (const result of tasks) {
    if (result.status === "fulfilled") discovered.push(...result.value);
    else { failed += 1; console.error("NEWAPP discovery", result.reason); }
  }
  const current = await db().from("newapp_systems").select("id,api_brand,api_station_id,api_device_sn");
  if (current.error) throw current.error;
  const existing = new Map((current.data ?? []).map((row) => [`${row.api_brand}:${row.api_station_id}`, row]));
  let inserted = 0;
  for (const system of discovered) {
    ...
    if (!row) {
      const result = await db().from("newapp_systems").insert({
        ...system, is_active: true, polling_enabled: true, polling_interval_min: system.api_brand === "ENPHASE" ? 15 : 5,
      });
      if (!result.error) inserted += 1;
    } else if (!row.api_device_sn && system.api_device_sn) {
      await db().from("newapp_systems").update({ api_device_sn: system.api_device_sn, updated_at: new Date().toISOString() }).eq("id", row.id);
    }
  }
  return {
...
export async function executeNewappTask(task: string): Promise<TaskResult> {
  switch (task) {
    case "poll_fast": return await pollFast();
    case "poll_enphase": return await pollEnphase();
    case "consolidate_today": return await consolidate("today");
    case "consolidate_yesterday": return await consolidate("yesterday");
    case "aggregate_monthly": return await aggregateMonthly();
    case "uva": return await updateUva();
    case "morning_report": return await sendMorningReport();
    case "discovery": return await discoverSystems();
    default: throw new Error(`Unknown NEWAPP task: ${task}`);
  }
}
